...
class UserService:
    ...
    async def register_user(self, dto: UserRegister) -> UserBrief:
        async with self.uow:
            ...
            default_roles = await self.uow.users.get_default_roles()
            ...
            try:
                await self.uow.users.save(db_user)
    ...
    async def authenticate_user(self, email: str, password: str) -> UUID:
        async with self.uow:
            db_user = await self.uow.users.get_by_email(email)
    ...
    async def update_user(self, user_id: UUID, dto: UserUpdate) -> UserBrief:
        async with self.uow:
            ...
            try:
                await self.uow.users.update(db_user, update_data)
            except RepoUniqueViolationError as e:
